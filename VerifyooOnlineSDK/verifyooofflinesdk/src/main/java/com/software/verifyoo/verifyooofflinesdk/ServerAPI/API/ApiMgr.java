...
public class ApiMgr {
    ...
    private void ConvertTemplate(Template templateReg, ExpTemplate tempExpTemplate) {
        ...
        for (int idxGesture = 0; idxGesture < templateReg.Gestures.size(); idxGesture++) {
            tempGesture = templateReg.Gestures.get(idxGesture);

            tempExpShape = new ExpShape();
            tempExpShape.Instruction = "";
            tempExpShape.Strokes = new ArrayList<>();

            for (int idxStroke = 0; idxStroke < tempGesture.Strokes.size(); idxStroke++) {
                tempStroke = tempGesture.Strokes.get(idxStroke);
