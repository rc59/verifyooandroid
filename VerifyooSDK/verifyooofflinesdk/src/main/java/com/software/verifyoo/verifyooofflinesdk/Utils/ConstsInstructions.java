package com.software.verifyoo.verifyooofflinesdk.Utils;

/**
 * Created by roy on 6/13/2016.
 */
public class ConstsInstructions {

    public static final String INSTRUCTION_CODE_BLETTER = "BLETTER";
    public static final String INSTRUCTION_CODE_DLETTER = "DLETTER";
    public static final String INSTRUCTION_CODE_ELETTER = "ELETTER";
    public static final String INSTRUCTION_CODE_FLETTER = "FLETTER";
    public static final String INSTRUCTION_CODE_KLETTER = "KLETTER";
    public static final String INSTRUCTION_CODE_MLETTER = "MLETTER";
    public static final String INSTRUCTION_CODE_PLETTER = "PLETTER";
    public static final String INSTRUCTION_CODE_RLETTER = "RLETTER";
    public static final String INSTRUCTION_CODE_SLETTER = "SLETTER";
    public static final String INSTRUCTION_CODE_ALETTER = "ALETTER";

    public static final String INSTRUCTION_CODE_GETTER = "GLETTER";
    public static final String INSTRUCTION_CODE_HLETTER = "HLETTER";
    public static final String INSTRUCTION_CODE_ILETTER = "ILETTER";
    public static final String INSTRUCTION_CODE_JLETTER = "JLETTER";
    public static final String INSTRUCTION_CODE_QLETTER = "QLETTER";
    public static final String INSTRUCTION_CODE_TLETTER = "TLETTER";
    public static final String INSTRUCTION_CODE_WLETTER = "WLETTER";
    public static final String INSTRUCTION_CODE_YLETTER = "YLETTER";
    public static final String INSTRUCTION_CODE_ZLETTER = "ZLETTER";
    public static final String INSTRUCTION_CODE_NLETTER = "NLETTER";

    public static final String INSTRUCTION_CODE_CLETTER = "CLETTER";
    public static final String INSTRUCTION_CODE_LLETTER = "LLETTER";
    public static final String INSTRUCTION_CODE_OLETTER = "OLETTER";
    public static final String INSTRUCTION_CODE_ULETTER = "ULETTER";
    public static final String INSTRUCTION_CODE_VLETTER = "VLETTER";
    public static final String INSTRUCTION_CODE_XLETTER = "XLETTER";

    public static final String INSTRUCTION_STRING_B = "B";
    public static final String INSTRUCTION_STRING_D = "D";
    public static final String INSTRUCTION_STRING_E = "E";
    public static final String INSTRUCTION_STRING_F = "F";
    public static final String INSTRUCTION_STRING_K = "K";
    public static final String INSTRUCTION_STRING_M = "M";
    public static final String INSTRUCTION_STRING_P = "P";
    public static final String INSTRUCTION_STRING_R = "R";
    public static final String INSTRUCTION_STRING_S = "S";
    public static final String INSTRUCTION_STRING_A = "A";

    public static final String INSTRUCTION_STRING_G = "G";
    public static final String INSTRUCTION_STRING_H = "H";
    public static final String INSTRUCTION_STRING_I = "I";
    public static final String INSTRUCTION_STRING_J = "J";
    public static final String INSTRUCTION_STRING_Q = "Q";
    public static final String INSTRUCTION_STRING_T = "T";
    public static final String INSTRUCTION_STRING_W = "W";
    public static final String INSTRUCTION_STRING_Y = "Y";
    public static final String INSTRUCTION_STRING_Z = "Z";
    public static final String INSTRUCTION_STRING_N = "N";

    public static final String INSTRUCTION_STRING_C = "C";
    public static final String INSTRUCTION_STRING_L = "L";
    public static final String INSTRUCTION_STRING_O = "O";
    public static final String INSTRUCTION_STRING_U = "U";
    public static final String INSTRUCTION_STRING_V = "V";
    public static final String INSTRUCTION_STRING_X = "X";
}
