...
//        idxSelectedWord = UtilsGeneral.IdxCurrentWord;
        UtilsGeneral.IdxCurrentWord++;
        if(UtilsGeneral.IdxCurrentWord >= mListWords.size()) {
        ...
        for(int idxChar = 0; idxChar < selectedWord.length(); idxChar++) {
            ...
            for(int idxInstruction = 0; idxInstruction < Consts.TOTAL_NUM_GESTURES; idxInstruction++) {
                ...
                if (UtilsConvert.InstructionCodeToInstruction(tempInstruction).toLowerCase().compareTo(tempChar.toLowerCase()) == 0) {
